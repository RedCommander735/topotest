 ...
 */
export function hideElementOverflow({element, CONTAINER_WITH_OVERFLOW_HIDDEN_CSS_CLASS= 'xyzw-container'}: {element : HTMLElement; CONTAINER_WITH_OVERFLOW_HIDDEN_CSS_CLASS?: string}): ZoomDragHTMLElement {
  ...
  if (!element?.parentElement?.classList.contains(CONTAINER_WITH_OVERFLOW_HIDDEN_CSS_CLASS)) {
    element?.parentElement?.classList.add(CONTAINER_WITH_OVERFLOW_HIDDEN_CSS_CLASS);
  }
